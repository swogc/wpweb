...
jQuery(document).ready(function($){
	...
	$('#section-of_demo .button-primary').live('click', function(e) {
		var confirm = window.confirm('WARNING: Import demo contents such as posts, menus, pages, widgets. Please wait for a 1-2 minutes until it completes the importing process.');

		if(confirm == false) {
			return false;
		} else {
			
			jQuery('.button-primary.import-demo').hide();
			jQuery('#section-of_demo .explain').hide();
			jQuery('#section-of_demo .controls').addClass('preloader')
			return true;
		}
	});
