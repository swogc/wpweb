...
if ( ! function_exists( 'woocommerce_template_loop_product_thumbnail' ) ) {
	function woocommerce_template_loop_product_thumbnail() {
		...
		switch ($evont_data['woo_image_hover']){
...
function evont_get_current_cart_info() {
	global $woocommerce;
...
function evont_add_to_cart_success_ajax( $count_cat_product ) {
	global $woocommerce;
	list( $cart_items ) = evont_get_current_cart_info();
	if ( $cart_items > 0 ) {
...
add_filter( 'add_to_cart_fragments', 'evont_add_to_cart_success_ajax' );
...
add_filter( 'loop_shop_per_page', 'evont_loop_shop_per_page' );
function evont_loop_shop_per_page() {
	$evont_data =  evont_globals();
 	parse_str( $_SERVER['QUERY_STRING'], $params );
	if ( isset($evont_data['woo_number_per_page']) && $evont_data['woo_number_per_page'] ) {
		$per_page = $evont_data['woo_number_per_page'];
	} else {
		$per_page = 12;
	}
