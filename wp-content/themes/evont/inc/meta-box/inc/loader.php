...
class RWMB_Loader
{
	...
	protected function constants()
	{
		...
		define( 'evont_RWMB_VER', '4.8.7' );

		list( $path, $url ) = self::get_path( dirname( dirname( __FILE__ ) ) );

		// Plugin URLs, for fast enqueuing scripts and styles
		define( 'evont_RWMB_URL', $url );
		define( 'evont_RWMB_JS_URL', trailingslashit( evont_RWMB_URL . 'js' ) );
		define( 'evont_RWMB_CSS_URL', trailingslashit( evont_RWMB_URL . 'css' ) );

		// Plugin paths, for including files
		define( 'evont_RWMB_DIR', $path );
		define( 'evont_RWMB_INC_DIR', trailingslashit( evont_RWMB_DIR . 'inc' ) );
	}
	...
	public function init()
	{
		...
		require evont_RWMB_INC_DIR . 'autoloader.php';
		$autoloader = new RWMB_Autoloader;
		$autoloader->add( evont_RWMB_INC_DIR, 'RW_' );
		$autoloader->add( evont_RWMB_INC_DIR, 'RWMB_' );
		$autoloader->add( evont_RWMB_INC_DIR . 'fields', 'RWMB_', '_Field' );
		$autoloader->add( evont_RWMB_INC_DIR . 'walkers', 'RWMB_Walker_' );
		$autoloader->register();
		...
		// Public functions
		require evont_RWMB_INC_DIR . 'functions.php';
	}
}
