...
class RWMB_File_Field extends RWMB_Field
{
	...
	{
		wp_enqueue_style( 'rwmb-file', evont_RWMB_CSS_URL . 'file.css', array(), evont_RWMB_VER );
		wp_enqueue_script( 'rwmb-file', evont_RWMB_JS_URL . 'file.js', array( 'jquery' ), evont_RWMB_VER, true );
		...
		if ( ! $wp_scripts->get_data( 'rwmb-file', 'data' ) )
		{
			wp_localize_script( 'rwmb-file', 'rwmbFile', array(
				'maxFileUploadsSingle' => esc_html__( 'You may only upload maximum %d file', 'evont' ),
				'maxFileUploadsPlural' => esc_html__( 'You may only upload maximum %d files', 'evont' ),
			) );
		}
	}
	...
	public static function wp_ajax_delete_file()
	{
		...
		if ( $success )
			wp_send_json_success();
		wp_send_json_error( __( 'Error: Cannot delete file', 'evont' ) );
	}
	...
	{
		$i18n_title = apply_filters( 'rwmb_file_upload_string', _x( 'Upload Files', 'file upload', 'evont' ), $field );
		$i18n_more  = apply_filters( 'rwmb_file_add_string', _x( '+ Add new file', 'file upload', 'evont' ), $field );
	...
	{
		$i18n_delete = apply_filters( 'rwmb_file_delete_string', _x( 'Delete', 'file upload', 'evont' ) );
		$i18n_edit   = apply_filters( 'rwmb_file_edit_string', _x( 'Edit', 'file upload', 'evont' ) );
