...
class RWMB_Validation
{
	...
	{
		wp_enqueue_script( 'jquery-validate', evont_RWMB_JS_URL . 'jquery.validate.min.js', array( 'jquery' ), evont_RWMB_VER, true );
		wp_enqueue_script( 'rwmb-validate', evont_RWMB_JS_URL . 'validate.js', array( 'jquery-validate' ), evont_RWMB_VER, true );
		/**
		...
		if ( ! $wp_scripts->get_data( 'rwmb-validate', 'data' ) )
		{
			wp_localize_script( 'rwmb-validate', 'rwmbValidate', array(
				'summaryMessage' => __( 'Please correct the errors highlighted below and try again.', 'evont' ),
			) );
		}
	}
}
