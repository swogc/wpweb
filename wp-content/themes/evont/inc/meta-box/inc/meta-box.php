...
class RW_Meta_Box
{
	...
	public function enqueue()
	{
		...
		wp_enqueue_style( 'rwmb', evont_RWMB_CSS_URL . 'style.css', array(), evont_RWMB_VER );
		if ( is_rtl() )
			wp_enqueue_style( 'rwmb-rtl', evont_RWMB_CSS_URL . 'style-rtl.css', array(), evont_RWMB_VER );
		...
		foreach ( $this->fields as $field )
		{
			if ( $field['clone'] )
			{
				wp_enqueue_script( 'rwmb-clone', evont_RWMB_JS_URL . 'clone.js', array( 'jquery-ui-sortable' ), evont_RWMB_VER, true );
				break;
			}
		}
		...
		if ( $this->meta_box['autosave'] )
			wp_enqueue_script( 'rwmb-autosave', evont_RWMB_JS_URL . 'autosave.js', array( 'jquery' ), evont_RWMB_VER, true );
